...
from megatron.core.telemetry import span_cm
...
```python
from megatron.core.telemetry import managed_span, SpanGroup
from megatron.training.global_vars import get_telemetry
...
```python
from megatron.core.telemetry import get_tracer, span_cm
...
```python
from megatron.core.telemetry import is_span_group_enabled, SpanGroup
...
```bash
# OTel API only (already in dev/lts extras — no traces/metrics exported)
