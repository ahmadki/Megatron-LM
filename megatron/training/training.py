...
# ``managed_span`` and ``trace_fn`` are already no-ops when telemetry is disabled
# (they check is_span_group_enabled() at call time, which returns False until
# setup_telemetry() enables specific groups).
from megatron.core.telemetry._state import is_span_group_enabled as _otel_sg_enabled
from megatron.core.telemetry.helpers import managed_span as _otel_managed_span
from megatron.core.telemetry.helpers import safe_set_span_attributes as _otel_safe_set_attrs
from megatron.core.telemetry.helpers import trace_fn as _otel_trace_fn
...
def pretrain(
    ...
    if _otel_sg_enabled('job'):
        ...
        from megatron.core.telemetry.helpers import safe_set_span_attributes as _otel_set_attrs
...
def train_step(forward_step_func, data_iterator, model, optimizer, opt_param_scheduler, config, forward_backward_func, iteration=None):
    ...
    if _otel_sg_enabled('forward_backward') or _otel_sg_enabled('optimizer'):
        from megatron.core.telemetry.helpers import span_cm, safe_set_span_attributes as _otel_set_attrs
...
def training_log(
    ...
    if iteration % args.log_interval == 0 or is_first_iteration:
        ...
        if _otel_telemetry_log is not None and _otel_telemetry_log.is_exporting:
            from megatron.core.telemetry._instruments import record_training_metrics
