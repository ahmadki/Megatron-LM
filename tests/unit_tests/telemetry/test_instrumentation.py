...
from megatron.core.telemetry.helpers import span_cm
...
class TestDisabledTelemetry:
    ...
    def test_setup_telemetry_disabled_noop_tracing(self):
        """Disabled telemetry: spans created via handle should be no-ops."""
        cfg = TelemetryConfig(enabled=False)
        handle = setup_telemetry(cfg, rank=0, world_size=1)

        # Span creation should not raise and span should be non-recording
        with handle.tracer.start_as_current_span("test") as span:
            # A no-op span may or may not be "recording" depending on API version;
            # the key contract is that no exception is raised.
            pass
